...
class AssessmentApi {
  ...
  static async createAssessment(userId) {
    return (await ApiClient.post("/api/assessment", { user_id: userId })).json();
  }
...
class QuestionApi {
  static async getQuestionsWithOptions() {
    return (await ApiClient.get("/api/question?includeOptions=true")).json();
  }
...
class AssessmentFlow {
  ...
  static async getNextQuestion(assessmentId) {
    try {
      ...
      const answeredQuestions =
        await QuestionApi.getAnsweredQuestions(assessmentId);
  ...
  static async answerQuestion(assessmentId, questionId, optionId) {
    try {
      ...
      const nextQuestion = await AssessmentFlow.getNextQuestion(assessmentId);
