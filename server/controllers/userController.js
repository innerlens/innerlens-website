...
export async function getUserById(req, res) {
	try {
		...
		return res
			...
			.json({ error: "Could not retrieve user", detail: err.message });
	}
}

export async function getUserByGoogleId(req, res) {
	try {
		const user = await userRepository.findByKey(
			"google_sub",
			req.params.id
		);

		if (!user) {
			return res.status(HTTP_STATUS.OK).json({ userExists: false });
		}

		res.status(HTTP_STATUS.OK).json({ userExists: true, ...user });
	} catch (err) {
		console.error("Get user failed:", err.message);
		return res
			.status(HTTP_STATUS.INTERNAL_SERVER_ERROR)
...
export async function createUser(req, res) {
	try {
		...
		return res
			.status(HTTP_STATUS.INTERNAL_SERVER_ERROR)
			.json({ error: "Could not create user", detail: err.message });
	}
}
