...
export async function getAssessmentResponseById(req, res) {
	try {
		const response = await assessmentResponseRepository.findById(
			parseInt(req.params.id)
		);
		if (!response)
			return res
				.status(HTTP_STATUS.NOT_FOUND)
				.json({ error: "Assessment response not found" });
		...
		return res
			...
			.json({
				error: "Could not retrieve assessment response",
				detail: err.message,
			});
	}
}
...
export async function getAllAssessmentResponsesByAssessmentyId(req, res) {
	try {
		...
		console.error(
			"Get assessment responses by assessment id failed:",
			err.message
		);
		res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
			error: "Could not fetch responses for assessment id",
			detail: err.message,
		});
	}
}
...
export async function createAssessmentResponse(req, res) {
	try {
		...
		if (!(await questionOptionRepository.findById(question_option_id))) {
			return res
				.status(HTTP_STATUS.NOT_FOUND)
				.json({ error: "Question option does not exist" });
		}

		if (
			await assessmentResponseRepository.findResponseByAssessmentIdAndQuestionId(
				assessment_id,
				question_id
			)
		) {
			return res
				...
				.json({
					error: "Assessment response already exists for question id",
				});
		}

		const response = await assessmentResponseRepository.create({
			assessment_id,
			question_id,
			question_option_id,
		});
		...
		return res
			.status(HTTP_STATUS.INTERNAL_SERVER_ERROR)
			.json({
				error: "Could not create assessment response",
				detail: err.message,
			});
	}
}
...
export async function updateAssessmentResponse(req, res) {
	try {
		...
		const existingResponse = await assessmentResponseRepository.findById(
			id
		);
		if (!existingResponse) {
			return res
		...
		if (question_option_id !== undefined) {
			if (
				!(await questionOptionRepository.findById(question_option_id))
			) {
				return res
					.status(HTTP_STATUS.NOT_FOUND)
					.json({ error: "Question option does not exist" });
			}
			updateData.question_option_id = question_option_id;
		}

		const updatedResponse = await assessmentResponseRepository.update(
			id,
			updateData
		);
