...
export async function createAssessment(req, res) {
  ...
  export async function completeAssessment(req, res) {
    try {
      ...
      const updatedAssessment = await assessmentRepository.update(id, { completed_at });  
  ...
  try {
    ...
    const responses = await assessmentResponseRepository.findAllByKey('assessment_id', assessmentId);
    if (!responses.length) return res.status(HTTP_STATUS.NOT_FOUND).json({ error: 'No responses for this assessment' });

    const traitCounts = {};

    for (const response of responses) {
      const option = await questionOptionRepository.findById(response.question_option_id);
      if (option?.trait_id) {
        traitCounts[option.trait_id] = (traitCounts[option.trait_id] || 0) + 1;
      }
    }
